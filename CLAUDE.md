...
`research_brief.yaml` v5.0 implements the locked design from `docs/design-references/research_brief_reference.md`. **Research brief = approval gate. Research plan = execution doc. Brief approved → plan elaborates.** This is the FIRST document in a study lifecycle — no prior inputs exist. Modal simplified from 15 fields to 7 + study selector (fixes Scenario A silent-failure bug where file didn't land). Removed: hypotheses, success criteria, expected outcomes, next steps, user journeys, target barriers, team table, participant criteria (all belong in plan). Added: study selector, out of scope, timeline presets, decision deadline, optional budget line. AI tasks reduced from 8 to 5. Pattern C with Approval section exception (brief IS the approval gate). Handler: removed `isFromRequest` branching, added Slack profile lookup for lead_researcher, methodology label mapping.
