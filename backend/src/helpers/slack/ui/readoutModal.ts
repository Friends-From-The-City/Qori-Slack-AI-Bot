...
/** The shape of private_metadata for the readout_modal_submit modal. */
export interface ReadoutModalMetadata {
  selectedStudyId: number | string | null;
  selectedStudyName: string | null;
  selectedStudyPath: string | undefined | null;
  reportType: string;
  hasReadout: boolean;
  origin?: {
    team: string;
  ...
  /** Transient fields carried through modal updates. */
...
export const buildReadoutModal = (state: ReadoutModalState = {}) => {
  ...
  const getTargetedReadoutsConfig = (): Record<string, unknown>[] => {
