<!--
This is a locked design reference for the journey_mapping template.

Status: Locked April 30, 2026
Pattern: B (inline citation) per Section 6 of standards

Inputs and rationale (see Section 7 of standards doc):
  - Required: Session summaries (multiple) — primary source. Already structured
    per-participant with pain points, successes, insights extracted.
  - Recommended: Observer notes — captures behavioral observations across stages
    that participants don't articulate (pauses, hesitation, confusion).
  - Recommended: Affinity map — themes spanning participants help identify patterns
    ACROSS journey stages, not just within them.
  - Recommended: Research plan — provides study context (which app, which user type,
    what tasks) that defines journey scope.
  - Excluded: Coded transcripts — no longer in the research flow.
  - Excluded: Discussion guide — useful for journey stage definition during planning,
    but session summaries already reflect what was actually asked. Available as
    reference link only.
  - Excluded: Participant tracker — demographics/scheduling, not journey content.
-->
