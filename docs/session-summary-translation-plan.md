...
|-------------|---------------|--------|
| {% raw %}`# Session Summary — {{participant_id}}`{% endraw %} with multi-line blockquote (Study/Date) | `# Session Summary: PT-001` with single-line masthead | Prompt rewrite |
...
{% raw %}These insights emerged from observing {{participant_id}}'s session and connect specific behaviors to broader patterns worth investigating across the participant pool.{% endraw %}
...
| Current v1.5 | Locked design | Change |
|-------------|---------------|--------|
| {% raw %}`*Generated by Qori • {{current_date}}*`{% endraw %} in prompt | Backend footer (2-column table + italic line). Locked design shows a "Participant" row. | Remove from prompt |
...
|-------------|---------------|
| {% raw %}`{{participant_id}}-session-summary-{{session_date}}.md`{% endraw %} | {% raw %}`{{participant_id}}-session-summary-{{current_date_iso}}.md`{% endraw %} |
...
- Source data injection ({% raw %}`{{coded_transcript_content}}`{% endraw %}, {% raw %}`{{notes_content}}`{% endraw %})
