...
**Recommendation: Option A — {% raw %}`# Personas: {{selected_study}}`{% endraw %}**
...
Related Artifacts uses {% raw %}`{{detected_files}}`{% endraw %} ground truth.
...
| Current v4.2 | Locked design | Change |
|-------------|---------------|--------|
| {% raw %}`*Generated by Qori • {{current_date}}*`{% endraw %} in prompt | Backend footer | Remove from prompt |
...
|-------------|---------------|
| {% raw %}`{{selected_study}}_personas_{{current_date}}.md`{% endraw %} | {% raw %}`{{selected_study}}-personas-{{current_date_iso}}.md`{% endraw %} |
...
| Clean H1 | Yes | {% raw %}`# Personas: {{selected_study}}`{% endraw %} (no emoji) |
| Bold-em-dash methodology | Yes | Standard 4.3 |
| Appendix with artifacts + validity | Yes | New addition |
| {% raw %}`{{detected_files}}`{% endraw %} for artifacts | Yes | Already wired in synthesis handler |
...
3. Verify:
   ...
   - Appendix with Related Artifacts (from {% raw %}`{{detected_files}}`{% endraw %}) + Validity Checklist
   - No duplicate footer
