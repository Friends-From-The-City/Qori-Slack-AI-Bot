...
| 37 | "Turns your brief into a stakeholder-ready plan" (row 12 addendum) | `studySetupModal.ts:88` | Row 12 marked TRUE for required cascade variables. However, three tasks (`background`, `participant_glance`, `participant_composition_prose`) used optional brief variables with no guards — when absent, LLM fabricated content using template examples as source material. | FALSE → FIXED (skip-when guards PR) |
