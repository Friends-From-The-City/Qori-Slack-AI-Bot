...
**New (from locked design):**
- Related Artifacts: session summaries (per participant), observer notes, affinity map, research plan — using real paths from {% raw %}`{{detected_files}}`{% endraw %}
...
| Current v3.11 | Locked design | Change |
|--------------|---------------|--------|
| {% raw %}`*Generated by Qori • {{current_date}}*`{% endraw %} in prompt | Backend footer (Document Information table) | Remove from prompt |
...
|--------------|----------------|
| {% raw %}`{{selected_study}}-journey-mapping-{{current_date_iso}}.md`{% endraw %} | {% raw %}`{{selected_study}}-journey-map-{{current_date_iso}}.md`{% endraw %} |
...
- Source data references ({% raw %}`{{combined_file_content}}`{% endraw %}, {% raw %}`{{selected_study}}`{% endraw %}, {% raw %}`{{focus_area}}`{% endraw %})
...
| Appendix artifact paths use {% raw %}`{{detected_files}}`{% endraw %} | Already wired | Backend change deployed for synthesis handler |
