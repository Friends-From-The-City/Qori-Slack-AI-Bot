...
Four streams executed in order: Bolt typing → test infrastructure → e2e tests + pattern enforcement → architecture audit.

- **Stream 1 (Bolt handler typing):** 37 files modified. Custom wrapper types (`ViewSubmissionContext`, `SlashCommandContext`, `BlockActionContext`, `EventContext`) replaced with Bolt's native `SlackXxxMiddlewareArgs<T> & AllMiddlewareArgs`. 112 registration casts in events.ts → 1 (documented `view_closed` gap, ADR 0015). ~45 handler-body `(body as any)` casts eliminated. `any` count: 413 → 193. Remaining 193 are Slack API iteration (65), Block Kit manipulation (50), ack response_action (10), options handlers (15), infrastructure (5), Bolt gap (1).
- **Stream 2 (test infrastructure):** `docker-compose.test.yml` for containerized test DB. Also supports local Homebrew Postgres. `jest.integration.config.js` with globalSetup running all 33 migrations. Per-test truncation via `truncateAll()`. Mocks for `jsonwebtoken`/`nodemailer` to break user.model.ts transitive dependency chain.
- **Stream 3 (e2e + pattern enforcement):** 5 critical flow tests (25 assertions), 5 pattern enforcement tests (9 assertions). All 10 deliberately broken and verified to fail. Deliberate-break verification surfaced 2 pattern test weaknesses (budget counting only `as any` not `: any`; TemplateContractError check defeated by comment lines) — fixed in place.
- **Stream 4 (architecture audit):** Full audit per `docs/audits/quarterly-architecture-audit.md`. Report at `docs/audits/2026-Q2-audit-post-migration.md`. All 7 ADRs conform. Filed ADR 0015 (Bolt native types).
